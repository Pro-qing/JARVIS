...
void TIM1_Cap_Init(u16 arr, u16 psc)
{
	...
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource8,GPIO_AF_TIM1); //PB0复用位定时器3
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource9,GPIO_AF_TIM1); //PB0复用位定时器3
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource11,GPIO_AF_TIM1); //PB0复用位定时器3
...
{ 
	...
	//清除中断标志位 
	//TIM_ClearITPendingBit(TIM1, TIM1_UP_TIM10_IRQn); //Clear the interrupt flag bit //清除中断标志位
