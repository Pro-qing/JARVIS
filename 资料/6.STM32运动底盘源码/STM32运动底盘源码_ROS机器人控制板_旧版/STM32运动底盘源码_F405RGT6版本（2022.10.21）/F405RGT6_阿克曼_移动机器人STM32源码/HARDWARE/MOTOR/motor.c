...
{		 					 
  ...
  //Enable GPIO peripheral clock //使能GPIO外设时钟
	...
	GPIO_PinAFConfig(GPIOC,GPIO_PinSource6,GPIO_AF_TIM8);   //复用为TIM2 编码器接口
  GPIO_PinAFConfig(GPIOC,GPIO_PinSource7,GPIO_AF_TIM8);   //复用为TIM2 编码器接口
...
{
GPIO_InitTypeDef GPIO_InitStructure;
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA, ENABLE); //Enable port clock  //使能端口时钟
